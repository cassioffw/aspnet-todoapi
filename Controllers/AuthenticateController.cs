...
public class AuthenticateController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateUserAsync([FromBody] CreateUserModel model)
    {
        ...
        {
            return StatusCode(StatusCodes.Status500InternalServerError, new ResponseModel { Sucess = false, Message = "Usuario já existe!" });
        }
        ...
        if (!result.Succeeded)
        {
            return StatusCode(StatusCodes.Status500InternalServerError, new ResponseModel { Sucess = false, Message = "Erro ao criar usuario" });
        }
